...
app.post('/api/blacklist', async (req, res) => {
    ...
    try {
        if (type === 'dl') {
            await licenseCollection.updateOne({ dl_number: cleanedNumber }, { $set: { Verification: "blacklisted" } }, { upsert: true });
            res.json({ message: `Driving License ${cleanedNumber} added to blacklist.` });
        } else if (type === 'rc') {
            await rcCollection.updateOne({ regn_number: cleanedNumber }, { $set: { verification: "blacklisted" } }, { upsert: true });
...
async function getANPRDataFromPython(imagePath) {
    try {
        ...
        return response.data; // Return the full object { plate_number, raw_text }
    } catch (error) {
...
async function getDLOCRFromPython(imagePath) {
    try {
        ...
        return response.data.dl_numbers?.[0] || null; // Return first number or null
    } catch (error) {
...
app.post('/api/ocr/dl', upload.single('dlImage'), async (req, res) => {
    ...
    try {
        ...
        if (dlImage) fs.unlinkSync(dlImage.path);
    }
});
...
app.post('/api/ocr/rc', upload.single('rcImage'), async (req, res) => {
    ...
    try {
        ...
        // Send back the cleaned plate number for the user to verify/edit
        ...
        if (rcImage) fs.unlinkSync(rcImage.path);
    }
});


// --- MODIFIED --- Simplified Verify Endpoint (no longer handles file uploads)
app.post('/api/verify', async (req, res) => {
    // This endpoint now receives the final, user-corrected numbers as JSON
    ...
    try {
        // Fetch data based on the provided numbers
        if (dl_number) {
            dlData = await getDLData(dl_number);
        }
        if (rc_number) {
            rcData = await getRCData(rc_number);
        }
        
        // --- Consolidated Log Entry Logic ---
        const logEntry = {
            timestamp: new Date(),
            scanned_by: (dl_number || rc_number) ? 'OCR/Manual' : 'Manual',
        ...
        let suspicious = false; // This will be sent in the final response

        // --- ADDED --- Create a specific alert log if an item is blacklisted
        if (dlData?.status === 'blacklisted') {
            suspicious = true; // Mark transaction as suspicious
        ...
        if (rcData?.status === 'blacklisted') {
            ...
            await logsCollection.insertOne({
                ...
                description: `Blacklisted vehicle ${rcData.regn_number} was scanned. Potential crime involved.`,
                location: location || 'unknown',
                tollgate: tollgate || 'unknown',
                scanned_by: 'System Alert',
                suspicious: true
        ...
        // --- Suspicious DL Usage Check (existing logic) ---
        // This will now only run if the DL is NOT blacklisted to avoid redundant alerts
